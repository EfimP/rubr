...
var works = []Work{
	...
	{Date: time.Date(2025, 6, 20, 0, 0, 0, 0, time.UTC), Title: "НАЗВАНИЕ", Studentname: "ФИО"},
	{Date: time.Date(2025, 6, 15, 0, 0, 0, 0, time.UTC), Title: "НАЗВАНИЕ", Studentname: "ФИО"},
	{Date: time.Date(2025, 7, 5, 0, 0, 0, 0, time.UTC), Title: "НАЗВАНИЕ", Studentname: "ФИО"},
}
...
func AllWorksScreen() {
	...
	headerTitleText := canvas.NewText("Работы на проверку", headerTextColor)
	...
	for i, work := range works {
		...
		detailsButton := widget.NewButton("Проверить", func(w Work) func() {
			return func() {}
